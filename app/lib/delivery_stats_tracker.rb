...
class DeliveryStatsTracker
  ...
  def track_success!
    redis.incr(hourly_success_count_key(Time.zone.now))
  end

  def track_failure!
    redis.incr(hourly_failure_count_key(Time.zone.now))
  end

  def hourly_delivery_histories(start_time, end_time = Time.zone.now)
    period = 1.hour
    start_time = start_time.beginning_of_hour
    end_time = (end_time + period).beginning_of_hour

    Array.new((end_time - start_time).to_i / period) do |i|
  ...
  StatRecord = Struct.new(:time, :success_count, :failure_count)
